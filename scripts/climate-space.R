...
#pairs(cv[rsamp,])
par(mar=c(5,5,1,1))
plot(aet~cwd,data=cv[rsamp,],pch=19,col='gray',xlab='CWD (mm)',ylab='AET (mm)',cex.lab=2)
rsamp <- sample(1:nrow(CAcv),2000)
points(aet~cwd,data=CAcv[rsamp,],pch=19,col='lightblue')
points(aet~cwd,data=CAcv[rsamp,],pch=1,lwd=0.5)
